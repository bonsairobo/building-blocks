...
fn surface_nets_sine_sdf(c: &mut Criterion) {
    ...
    for diameter in [8, 16, 32, 64].iter() {
        ...
        let sample_extent =
            Extent3i::from_min_and_max(Point3i::fill(-radius), Point3i::fill(radius));
        ...
        group.bench_with_input(BenchmarkId::from_parameter(diameter), &(), |b, _| {
            b.iter(|| surface_nets(&samples, samples.extent(), 1.0, true, &mut buffer));
        });
    }
    group.finish();
}
